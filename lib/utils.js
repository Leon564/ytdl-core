...
// Callers embed the return value in an error message, so always resolve to a short
// name and never throw: saving the debug file is a convenience, and letting it fail
// loudly would replace the error being reported with a filesystem one.
exports.saveDebugFile = (name, body) => {
  ...
  try {
    ...
    console.warn(`\x1b[33mWARNING:\x1b[0m Could not save debug file "${filename}": ${err.message}`);
    return name;
  }
  return filename;
};
...
exports.applyPlayerClients = options => {
  if (!options.playerClients || options.playerClients.length === 0) {
    options.playerClients = ["ANDROID_VR", "WEB_EMBEDDED", "IOS", "ANDROID", "TV"];
  }
};
