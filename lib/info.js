...
const getWatchHTMLPage = async (id, options) => {
  ...
  try {
    ...
    // YouTube sometimes serves a watch page without the `ytInitialData` payload.
    // It only feeds optional extras (media, author, related videos, likes, chapters),
    // all of which already handle it being missing, so don't fail the whole request.
    try {
      info.response =
...
exports.getInfo = async (id, options) => {
  ...
  try {
    ...
    if (options.playerClients.includes("ANDROID_VR")) clientPromises.push(fetchAndroidVRPlayer(id, info, options));
...
const ANDROID_VR_CLIENT_VERSION = "1.57.29";
...
const fetchAndroidVRPlayer = async (videoId, info, options) => {
  const payload = {
  ...
  const opts = {
    requestOptions: {
      ...
      headers: {
        ...
        "User-Agent": `com.google.android.apps.youtube.vr.oculus/${ANDROID_VR_CLIENT_VERSION} (Linux; U; Android ${ANDROID_VR_OS_VERSION}; eureka-user Build/SQ3A.220605.009.A1) gzip`,
